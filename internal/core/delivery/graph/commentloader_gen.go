...
func (l *CommentLoader) Load(key int) ([]*model.Comment, error) {
...
func (l *CommentLoader) LoadThunk(key int) func() ([]*model.Comment, error) {
	l.mu.Lock()
...
func (l *CommentLoader) LoadAll(keys []int) ([][]*model.Comment, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *CommentLoader) LoadAllThunk(keys []int) func() ([][]*model.Comment, []error) {
	results := make([]func() ([]*model.Comment, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *CommentLoader) Prime(key int, value []*model.Comment) bool {
	l.mu.Lock()
	var found bool
	if _, found = l.cache[key]; !found {
		// make a copy when writing to the cache, its easy to pass a pointer in from a loop var
		// and end up with the whole cache pointing to the same value.
		cpy := make([]*model.Comment, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *CommentLoader) unsafeSet(key int, value []*model.Comment) {
	if l.cache == nil {
